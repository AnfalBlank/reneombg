...
import { Plus, Truck, Printer, X, Eye } from 'lucide-react'
...
import { useDeliveryOrders, useCreateDeliveryOrder, useConfirmDeliveryOrder, useInternalRequests, useDapur, useGudang, useItems } from '../../hooks/useApi'
...
export default function DeliveryOrderPage() {
    ...
    const [doItems, setDoItems] = useState<{ itemId: string; qty: number }[]>([{ itemId: '', qty: 1 }])
    ...
    useEffect(() => {
        if (form.irId) {
            ...
            if (ir) {
                ...
                setDoItems((ir.items || []).map((i: any) => ({ itemId: i.itemId, qty: i.qtyRequested - i.qtyFulfilled })))
            }
        }
    }, [form.irId])
    ...
    const handleSubmit = async () => {
        ...
        if (doItems.some(i => !i.itemId || i.qty <= 0)) {
    ...
    const handleConfirm = async (id: string, doNumber: string) => {
        try {
            ...
            success(`DO ${doNumber} berhasil dikonfirmasi dan stok diperbarui!`)
        } catch (e: any) {
            toastError(e?.message || 'Gagal konfirmasi DO.')
        }
    }
    ...
    return (
        <>
            <div className={`${styles.page} ${printingDo ? 'no-print' : ''}`}>
                                    ...
                                    <tr key={d.id}>
                                        ...
                                        <td>
                                            <div className={styles.rowActions}>
                                                ...
                                                <button className={styles.actionBtn} onClick={() => setPrintingDo(d)}>
                                                    <Printer size={12} /> Cetak SJ
                                                </button>
                                                {d.status === 'draft' && (
                ...
                {/* Create DO Modal */}
                <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Buat Delivery Order">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        ...
                        <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                ...
                                <button onClick={() => setDoItems(p => [...p, { itemId: '', qty: 1 }])} style={{ fontSize: 12, color: 'var(--color-primary)', background: 'none', border: 'none', cursor: 'pointer' }}>+ Tambah Baris</button>
                            </div>
                        ...
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 8, borderTop: '1px solid var(--color-border)' }}>
                            ...
                            <Button onClick={handleSubmit} disabled={createDO.isPending}>{createDO.isPending ? 'Menyimpan...' : 'Buat DO'}</Button>
                        </div>
                    </div>
                </Modal>
                    ...
                    {viewDO && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 13 }}>
                                ...
                                <div><span style={{ color: 'var(--color-text-muted)' }}>Tanggal:</span> {viewDO.createdAt ? new Date(viewDO.createdAt).toLocaleDateString('id-ID') : '-'}</div>
                                ...
                                {viewDO.ir && <div><span style={{ color: 'var(--color-text-muted)' }}>Ref IR:</span> <span style={{ fontFamily: 'monospace' }}>{viewDO.ir?.irNumber}</span></div>}
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <Button variant="secondary" icon={<Printer size={14} />} onClick={() => { setPrintingDo(viewDO); setViewDO(null) }}>Cetak Surat Jalan</Button>
                                {viewDO.status === 'draft' && (
